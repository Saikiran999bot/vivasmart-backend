...
app.post('/api/analyze', analyzeLimiter, async (req,res) => {
  try {
    ...
    const ed = req.body.examDetails || {};
    const examCtx = [ed.board,ed.examType,ed.department,ed.semester,ed.subject,ed.notes].filter(Boolean).join(' | ');
    const prompt = `You are an expert viva examiner. Analyze this student project and return ONLY valid JSON with:
- questions: array of 10 {q, a} objects — targeted viva questions with detailed answers tailored to the exam context below
- keywords: array of 20 strings — key technical terms the examiner will ask about
- diagrams: array of 5 {title, explanation} objects — important diagrams to prepare
- projectTitle: string — inferred project name
${examCtx ? 'EXAM CONTEXT (tailor all questions to this): ' + examCtx + '\n' : ''}PROJECT TEXT: ${text}`;
...
app.post('/api/chat', async (req,res) => {
  try {
    const { messages, projectContext, examDetails } = req.body;
    if (!messages || !Array.isArray(messages) || !messages.length)
      return res.status(400).json({ error: 'Messages required.' });
    const ed = examDetails || {};
    const examCtx = [ed.board,ed.examType,ed.department,ed.semester,ed.subject].filter(Boolean).join(' | ');
    const sys = `You are VivaSmart AI, a friendly expert viva coach for polytechnic students.
Exam context: ${examCtx || 'MSBTE Viva Voce'}
${projectContext ? 'Project: ' + projectContext : ''}
Rules: Give concise exam-focused answers (3-6 sentences max). Use simple English students can repeat in viva. For viva questions give a model answer AND one delivery tip. Be encouraging and stay on topic.`;
    const contents = [
      { role: 'user',  parts: [{ text: sys + '\n[CHAT BEGINS]' }] },
      { role: 'model', parts: [{ text: 'Ready! Ask me anything about your project or viva preparation.' }] },
    ];
    ...
    const gr = await fetch(GURL, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ contents, generationConfig: { temperature: 0.55, maxOutputTokens: 1024 } }),
    });
    if (!gr.ok) { const e=await gr.json().catch(()=>({})); return res.status(502).json({ error: `AI: ${e?.error?.message||'Unknown'}` }); }
    const gd = await gr.json();
    let reply = '';
    for (const p of (gd?.candidates?.[0]?.content?.parts||[])) { if (p.text) reply += p.text; }
    if (!reply) return res.status(502).json({ error: 'Empty AI response.' });
    return res.json({ success: true, reply: reply.trim() });
  } catch(err) { console.error('Chat:', err.message); return res.status(500).json({ error: 'Chat failed: ' + err.message }); }
});
