...
${text}`;
    ...
    try {
      ...
      if (match) { try { parsed = JSON.parse(match[0]); } catch (_) { return res.status(502).json({ error: 'Could not parse AI response.' }); } }
      else return res.status(502).json({ error: 'Could not parse AI response.' });
    }
...
app.get('/api/admin/stats', requireAdmin, async (req, res) => {
  try {
    const [
      ...
      supabase.from('users').select('*', { count: 'exact', head: true }).eq('subscribed', true),
      supabase.from('coupons').select('*', { count: 'exact', head: true }).eq('active', true),
      supabase.from('payments').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      supabase.from('payments').select('plan').eq('status', 'verified'),
