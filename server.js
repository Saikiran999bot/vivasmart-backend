...
// CORS — allow all origins (frontend is a static HTML file)
app.use(cors({
  origin: '*',
...
PROJECT TEXT: ${text}`;
    ...
    // Loop through all parts (thinking model returns multiple parts)
    let rawText = '';
...
app.post('/api/payments/submit', async (req, res) => {
  try {
    ...
    if (!upiRef || !upiRef.trim())  return res.status(400).json({ error: 'UPI Reference number is required.' });
    if (!mobile || !mobile.trim())  return res.status(400).json({ error: 'Mobile number is required.' });
    if (!plan)                      return res.status(400).json({ error: 'Plan is required.' });
    if (!email && !userId)          return res.status(400).json({ error: 'Please login before making a payment.' });

    // Check for duplicate UPI ref
    const { data: existing } = await supabase
      .from('payments').select('id').eq('upi_ref', upiRef.trim()).maybeSingle();
    if (existing) return res.status(400).json({ error: 'This UPI reference was already submitted.' });

    // Build insert payload
    const payload = {
      plan:       plan.toString(),
      plan_label: planLabel || (plan === '10' ? 'One-Time (1 Trial)' : 'Monthly Unlimited'),
      upi_ref:    upiRef.trim(),
    ...
    console.error('Payment submit crash:', err.message);
    return res.status(500).json({ error: 'Failed to submit payment: ' + err.message });
  }
});
